@tool
extends "res://addons/opencode_godot/engine_bridge/commands/base_command.gd"

const NodeUtils := preload("res://addons/opencode_godot/engine_bridge/utils/node_utils.gd")
const PropertyParser := preload("res://addons/opencode_godot/engine_bridge/utils/property_parser.gd")
...
func _cleanup_screenshot_files() -> void:
	var request_path := RuntimePaths.file(RuntimePaths.SCREENSHOT_REQUEST)
	var screenshot_path := RuntimePaths.file(RuntimePaths.SCREENSHOT)
...
func _cleanup_input_files() -> void:
	var commands_path := RuntimePaths.file(RuntimePaths.INPUT_COMMANDS)
...
func _cleanup_inspector_files() -> void:
	var request_path := RuntimePaths.file(RuntimePaths.GAME_REQUEST)
	var response_path := RuntimePaths.file(RuntimePaths.GAME_RESPONSE)
	if FileAccess.file_exists(request_path):
		DirAccess.remove_absolute(ProjectSettings.globalize_path(request_path))
