...
func _run_headless(params: Dictionary, target_args: Array, target: String) -> Dictionary:
	...
	_run_counter += 1
	var prefix := RuntimePaths.file("headless_%d_%d" % [OS.get_process_id(), _run_counter])
	...
	if shell.is_empty():
		...
		paths.cleanup()
		return error_internal("Could not prepare the headless runner script under the OpenCode runtime directory")
...
func _shell_invocation(godot_bin: String, godot_args: Array, paths: RunPaths, is_windows: bool) -> Array:
	...
	# runtime IPC request files. This marker tells the services to stand down.
	var body := ""
	if is_windows:
...
func _kill_process_tree(pid: int) -> void:
	...
	var script_path := RuntimePaths.file("kill_tree.sh")
