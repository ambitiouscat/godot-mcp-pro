@tool
extends RefCounted
class_name OpenCodeGodotRuntimeServiceController

const SERVICE_DEFINITIONS := {
	"screenshot": {
		"autoload": "MCPScreenshot",
		"script": "res://addons/opencode_godot/engine_bridge/runtime/mcp_screenshot_service.gd",
	},
	"input": {
		"autoload": "MCPInputService",
		"script": "res://addons/opencode_godot/engine_bridge/runtime/mcp_input_service.gd",
	},
	"inspector": {
		"autoload": "MCPGameInspector",
		"script": "res://addons/opencode_godot/engine_bridge/runtime/mcp_game_inspector_service.gd",
	},
}
...
func setup(plugin: EditorPlugin, session_owner_nonce: String, game_running_probe: Callable = Callable()) -> void:
	...
	var project_hash := preload("res://addons/opencode_godot/engine_bridge/bridge_protocol_v1.gd").project_hash(canonical_project)
	var journal_dir := "user://opencode_godot/runtime/%s" % project_hash
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(journal_dir))
	_journal_path = journal_dir.path_join("runtime-service-ownership.cfg")
	_journal_backup_path = _journal_path + ".backup"
...
func _recover_stale_owned_services() -> void:
	...
	var loaded_path := _journal_path
	if config.load(_journal_path) != OK:
		loaded_path = _journal_backup_path
		if config.load(_journal_backup_path) != OK:
			return
	if not config.has_section("owned"):
		return
	var recorded_project: String = config.get_value("session", "project_path", "")
	var current_project := preload("res://addons/opencode_godot/engine_bridge/bridge_protocol_v1.gd").canonicalize_project_path(ProjectSettings.globalize_path("res://"))
	...
	if loaded_path == _journal_backup_path and FileAccess.file_exists(_journal_path):
...
func _save_journal() -> bool:
	...
	config.set_value(
		"session",
		"project_path",
		preload("res://addons/opencode_godot/engine_bridge/bridge_protocol_v1.gd").canonicalize_project_path(ProjectSettings.globalize_path("res://"))
	)
	for service_id: String in _owned:
		config.set_value("owned", service_id, (_owned[service_id] as Dictionary)["script"])
	var temporary := "%s.tmp-%s" % [_journal_path, owner_nonce]
	var save_error := config.save(temporary)
	if save_error != OK:
		return false
	var absolute_temporary := ProjectSettings.globalize_path(temporary)
	var absolute_journal := ProjectSettings.globalize_path(_journal_path)
	var absolute_backup := ProjectSettings.globalize_path(_journal_backup_path)
	if FileAccess.file_exists(_journal_backup_path):
		var remove_backup_error := DirAccess.remove_absolute(absolute_backup)
		if remove_backup_error != OK:
			DirAccess.remove_absolute(absolute_temporary)
			return false
	if FileAccess.file_exists(_journal_path):
	...
	if rename_error != OK:
		DirAccess.remove_absolute(absolute_temporary)
		if FileAccess.file_exists(_journal_backup_path) and not FileAccess.file_exists(_journal_path):
...
func _remove_journal() -> void:
	if _journal_path.is_empty():
		return
	if FileAccess.file_exists(_journal_path):
		DirAccess.remove_absolute(ProjectSettings.globalize_path(_journal_path))
	if FileAccess.file_exists(_journal_backup_path):
		DirAccess.remove_absolute(ProjectSettings.globalize_path(_journal_backup_path))
