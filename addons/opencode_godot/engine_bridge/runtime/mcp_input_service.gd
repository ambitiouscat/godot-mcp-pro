...
func _ready() -> void:
	_commands_path = RuntimePaths.file(RuntimePaths.INPUT_COMMANDS)
...
func _process_commands() -> void:
	var file := FileAccess.open(_commands_path, FileAccess.READ)
	if file == null:
		return
	var text := file.get_as_text()
	file.close()
	DirAccess.remove_absolute(ProjectSettings.globalize_path(_commands_path))
