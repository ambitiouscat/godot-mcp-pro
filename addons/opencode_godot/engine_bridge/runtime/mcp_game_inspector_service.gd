...
func _ready() -> void:
	_request_path = RuntimePaths.file(RuntimePaths.GAME_REQUEST)
	_response_path = RuntimePaths.file(RuntimePaths.GAME_RESPONSE)
	_debugger_continue_path = RuntimePaths.file(RuntimePaths.DEBUGGER_CONTINUE)
...
func _process(_delta: float) -> void:
	...
	if _pending_command and not FileAccess.file_exists(_request_path) and not FileAccess.file_exists(_response_path):
		...
		var flag := FileAccess.open(_debugger_continue_path, FileAccess.WRITE)
		if flag:
			flag.close()
...
func _write_response(data: Dictionary) -> void:
	...
	var file := FileAccess.open(_response_path, FileAccess.WRITE)
	if file:
